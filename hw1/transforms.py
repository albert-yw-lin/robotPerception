...
def transform_concat(t1, t2):
    ...
    """
    # TODO:
    if not transform_is_valid(t1): raise ValueError('t1 is invalid.')
    if not transform_is_valid(t2): raise ValueError('t2 is invalid.')
...
def transform_point3s(t, ps):
    ...
    if not (np.shape(ps)[-1] == 3 and ps.ndim == 2): raise ValueError('ps does not have correct shape.')
    homo_ones = np.ones((np.shape(ps)[0], 1))
    homo_ps = np.column_stack((ps, homo_ones)) #should be in the shape of (n, 4)
    homo_transformed = np.matmul(t, homo_ps.T) #should be in the shape of (4, n)
    return homo_transformed[:3].T #(n, 3)
