...
class TSDFVolume:
    ...
    *******************************************************************************
    ...
    def voxel_to_world(volume_origin, voxel_coords, voxel_size):
        ...
        # TODO
        return voxel_coords*voxel_size + volume_origin
    ...
    def transfrom_wrd2cam(camera_pose, voxel_world_coords):
        ...
        # TODO
        return transform_point3s(transform_inverse(camera_pose), voxel_world_coords)
    ...
    def compute_tsdf(voxel_z, valid_points, depth_image, valid_pixels, truncation_margin): 
        ...
        # TODO
        valid_pixels_depth = depth_image[valid_pixels[:, 1], valid_pixels[:, 0]]
        valid_voxels_depth = voxel_z[valid_points]
        d_proj = valid_pixels_depth - valid_voxels_depth
        tsdf_new = np.maximum(-1, np.minimum(1, d_proj/truncation_margin))
        return tsdf_new
    ...
    def update_tsdf(tsdf_old, tsdf_new, color_old, color_new):
        ...
        #TODO
        mask = np.abs(tsdf_new) < np.abs(tsdf_old)
        tsdf_old[mask] = tsdf_new[mask]
        color_old[mask] = color_new[mask]
        return tsdf_old, color_old
        # mask = np.abs(tsdf_new) >= np.abs(tsdf_old)
        # tsdf_new[mask] = tsdf_old[mask]
        # color_new[mask] = color_old[mask]
